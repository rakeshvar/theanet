...
def init_wts(wts, rand_gen, sizeW, sizeB, fan_in, fan_out, actvn):
    if wts is None:
        w_bound    = np.sqrt(6. / (fan_in + fan_out))
        w_values   = np.asarray(
            rand_gen.uniform(low=-w_bound, high=w_bound, size=sizeW),
            dtype=theano.config.floatX)
        b_values   = np.zeros(sizeB, dtype=theano.config.floatX)
    ...
    if wts and isSharedVar(wts[0]):     # Usually true when getting TestVersion
        W, b = wts
    else:
...
class HiddenLayer(Layer):
    def __init__(self, inpt, wts, rand_gen=None, n_in=None, n_out=None, pdrop=0,
        ...
        try: fan_in_out = n_in + n_out
        except TypeError: fan_in_out = None
...
class NeuralNet():
    def __init__(self, layers, training_params, allwts=None):
        ...
        if allwts is None: rand_gen = np.random.RandomState(training_params['SEED'])
        else:              rand_gen = None
        ...
        if layers[ilayer][0] == 'SoftmaxLayer':
            ...
            try:                centers = layers[ilayer][1].pop('centers')
            except KeyError:    centers = None 
    ...
    def get_trin_model(self):
        ...
        for param in trn_prms:
            ...
            updates.append((param_update, 
                self.training_params['MOMENTUM']*param_update + (1. - self.training_params['MOMENTUM'])*T.grad(cost, param)))
    ...
    def set_rate(self):
        ...
        (1 + self.training_params['CUR_EPOCH'] / self.training_params['EPOCHS_TO_HALF_RATE']))
